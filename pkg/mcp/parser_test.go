...
func TestParseJSONRPC_ValidMessages(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := parser.ParseData(tt.data, ebpf.EventTypeWrite, 100, "writer")
			if err != nil {
				t.Fatalf("ParseData write failed: %v", err)
			}

			// Then read event
			msgs, err := parser.ParseData(tt.data, ebpf.EventTypeRead, 200, "reader")
			if err != nil {
...
func TestParseJSONRPC_AllSupportedMethods(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.method, func(t *testing.T) {
			...
			// Write then read
			_, err := parser.ParseData(data, ebpf.EventTypeWrite, 100, "writer")
			if err != nil {
				t.Fatalf("Write failed: %v", err)
			}

			msgs, err := parser.ParseData(data, ebpf.EventTypeRead, 200, "reader")
			if err != nil {
...
func TestParseJSONRPC_InvalidMessages(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = parser.ParseData(tt.data, ebpf.EventTypeRead, 200, "reader")
			if err == nil {
...
func TestParseData_WriteReadCorrelation(t *testing.T) {
	...
	t.Run("Normal flow", func(t *testing.T) {
		// Write event
		writeMsg, err := parser.ParseData(data, ebpf.EventTypeWrite, 100, "writer")
		if err != nil {
			t.Fatalf("Write failed: %v", err)
		}
		if len(writeMsg) != 0 {
		...
		// Read event
		readMsg, err := parser.ParseData(data, ebpf.EventTypeRead, 200, "reader")
		if err != nil {
	...
	t.Run("Read without write", func(t *testing.T) {
		...
		_, err := newParser.ParseData(data2, ebpf.EventTypeRead, 200, "reader")
		if err == nil {
...
{"jsonrpc":"2.0","method":"notifications/progress","params":{"value":50}}`)
	...
	// Write event
	_, err := parser.ParseData(multipleData, ebpf.EventTypeWrite, 100, "writer")
	if err != nil {
		t.Fatalf("Write failed: %v", err)
	}

	// Read event
	msgs, err := parser.ParseData(multipleData, ebpf.EventTypeRead, 200, "reader")
	if err != nil {
		t.Fatalf("Read failed: %v", err)
	}
